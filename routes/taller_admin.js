...
router.put('/perfil', isAuthenticated, async (req, res) => {
    ...
    try {
        const { nombre, direccion, email, telefono, link_maps, foto_perfil, horario, sobre_nosotros } = req.body;
        ...
        let paramsTaller = [nombre || '', direccion || '', link_maps || '', horario || null, sobre_nosotros || ''];
...
router.post('/pos/venta', isAuthenticated, async (req, res) => {
    let connection;
    try {
        const { idItem, cantidad, metodoPago } = req.body;
        ...
        await connection.query(
            'INSERT INTO venta_fisica (idTaller, idItem, nombreProducto, cantidad, precioUnitario, total, metodo_pago) VALUES (?, ?, ?, ?, ?, ?, ?)',
            [admin[0].idTaller, idItem, item[0].nombre, cantidad, item[0].precio, total, metodo]
        );
...
router.post('/pos/cita/:id/cobrar', isAuthenticated, async (req, res) => {
    try {
        ...
        await db.query('UPDATE cita SET codigo_pago_efectivo = ?, metodo_pago = ?, monto = ? WHERE idCita = ?', [codigo, metodo, monto || 0, idCita]);
...
router.get('/reportes', isAuthenticated, async (req, res) => {
    try {
        ...
        if (tipo === 'dia' && fecha) {
            dateFilter = 'AND DATE(fecha) = ?';
            params.push(fecha);
        } else if (tipo === 'mes' && fecha) {
            dateFilter = 'AND DATE_FORMAT(fecha, \'%Y-%m\') = ?';
            params.push(fecha);
        } else if (tipo === 'anio' && fecha) {
            dateFilter = 'AND YEAR(fecha) = ?';
            params.push(fecha);
        }

        // 1. Ventas físicas POS
        let ventasQuery = `SELECT id, nombreProducto, cantidad, precioUnitario, total, metodo_pago, fecha FROM venta_fisica WHERE idTaller = ? ${dateFilter} ORDER BY fecha DESC`;
        const [ventasFisicas] = await db.query(ventasQuery, params);

        // 2. Reparaciones (citas cobradas)
        let citaParams = [idTaller];
        let citaDateFilter = '';
        if (tipo === 'dia' && fecha) {
            citaDateFilter = 'AND DATE(fechaHora) = ?';
            citaParams.push(fecha);
        } else if (tipo === 'mes' && fecha) {
            citaDateFilter = 'AND DATE_FORMAT(fechaHora, \'%Y-%m\') = ?';
            citaParams.push(fecha);
        } else if (tipo === 'anio' && fecha) {
            citaDateFilter = 'AND YEAR(fechaHora) = ?';
            citaParams.push(fecha);
        }
        const [reparaciones] = await db.query(
            `SELECT c.idCita, u.nombre as clienteNombre, c.monto, c.metodo_pago, c.fechaHora
             FROM cita c JOIN usuario u ON c.idCliente = u.idUsuario
             WHERE c.idTaller = ? AND c.codigo_pago_efectivo IS NOT NULL ${citaDateFilter}
             ORDER BY c.fechaHora DESC`,
            citaParams
        );

        // 3. Ventas online (pedidos_catalogo)
        let webDateFilter = '';
        let webParams = [idTaller];
        if (tipo === 'dia' && fecha) {
            webDateFilter = "AND DATE(pc.fecha_creacion) = ?";
            webParams.push(fecha);
        } else if (tipo === 'mes' && fecha) {
            webDateFilter = "AND DATE_FORMAT(pc.fecha_creacion, '%Y-%m') = ?";
            webParams.push(fecha);
        } else if (tipo === 'anio' && fecha) {
            webDateFilter = 'AND YEAR(pc.fecha_creacion) = ?';
            webParams.push(fecha);
        }
        // Try to find orders that contain products from this taller via JSON items
        let ventasWeb = [];
        try {
            const [allOrders] = await db.query(
                `SELECT pc.id, pc.fecha_creacion as fecha, pc.monto_total as total, pc.estado_pago as estado, pc.items_json
                 FROM pedidos_catalogo pc
                 WHERE pc.estado_pago NOT IN ('cancelled','Cancelado') ${webDateFilter}
                 ORDER BY pc.fecha_creacion DESC`,
                webParams.slice(1).length ? webParams.slice(1) : []
            );
            // Filter orders that contain items from this taller
            if (allOrders.length > 0) {
                const [tallerItems] = await db.query('SELECT idItem FROM iteminventario WHERE idTaller = ?', [idTaller]);
                const myItemIds = new Set(tallerItems.map(i => i.idItem));
                ventasWeb = allOrders.filter(order => {
                    try {
                        const items = typeof order.items_json === 'string' ? JSON.parse(order.items_json) : (order.items_json || []);
                        return Array.isArray(items) && items.some(it => myItemIds.has(it.idItem || it.id));
                    } catch { return false; }
                });
            }
        } catch (webErr) {
