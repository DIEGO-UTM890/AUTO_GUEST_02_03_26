    ...
    try {
        ...
        if (tallerRows.length > 0 && tallerRows[0].horario) {
            try {
                ...
                const dateObj = new Date(`${fecha}T12:00:00`); // 12:00 para evitar problemas de zona horaria al sacar el día
                const dayName = dayNames[dateObj.getDay()];

                const dayConfig = schedule[dayName];
                if (dayConfig) {
                    if (dayConfig.closed) {
                        return res.status(400).json({ error: `El taller está cerrado los días ${dayName}.` });
                    }
                ...
                // Si falla el parseo, dejamos pasar para no bloquear citas por error de data
            }
        }
        // ------------------------------------
